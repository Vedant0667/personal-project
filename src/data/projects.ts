...
export const projects: ProjectProps[] = [
  ...
  },
  {
    ...
    featured: false,
    summary:
      "A CHF hospital-at-home tool that scores discharge readiness and catches deterioration from vitals trajectories, not single thresholds. 2nd of 13 teams at the hackathon.",
    story:
      "Canary closes the gap between hospital discharge and home recovery for congestive heart failure patients. It runs a three-stage loop: an AI reads the discharge packet and a deterministic rules engine scores home-readiness (every number traceable on screen), the structured record hands off to the homecare EMR over FHIR, and a monitoring dashboard tracks daily vitals, firing a trajectory-based alert on day 4 before any single vital looks alarming. Accept and Override carry equal weight so the interface never nudges the clinical call, and it runs fully offline. I was the developer responsible for all technical work at the hackathon, where Canary placed 2nd of 13 teams.",
    bullets: [
      { strong: "Readiness scoring", text: "A deterministic rules engine scores home-readiness from an AI-extracted discharge packet; every number is traceable on screen" },
      { strong: "Interoperable", text: "The structured record hands off to the homecare EMR over a FHIR exchange" },
      { strong: "Trajectory alerts", text: "The monitoring dashboard escalates on multi-signal vital trends, before any single threshold is breached" },
